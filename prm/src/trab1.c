...
int main(int argc, const char **argv)
{
  ...
  client = playerc_client_create(NULL, "localhost", 6665 + playerIncrement);
  //client = playerc_client_create(NULL, "192.168.181.51", 6665 + playerIncrement);
  ...
  while(1)
  {
    ...
    if(state == FOLLOW_WAYPOINT || state == RETURN_TO_DEST) {
      ...
      playerc_position2d_set_cmd_vel(position2d, 0.4, 0, turn_speed, 1);
